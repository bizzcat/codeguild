...
def identify_highest_rainfall(clean_data_dict):
    highest_rainfall = max(clean_data_dict.values())
    return highest_rainfall
def identify_day_with_highest_rainfall(highest_rainfall, clean_data_dict):
    for k in clean_data_dict:
        if clean_data_dict[k] == highest_rainfall:
            day_with_with_highest_rainfall = k
    return day_with_with_highest_rainfall
...
def assign_day_averages(days_empty_values, clean_data_dict):
    ...
    for line in clean_data_list:
        ...
        day_averages[date] = (sum(days_empty_values[date]) / len(days_empty_values[date]))
    return day_averages
...
def assign_monthly_averages(months_empty_values, clean_data_list, clean_data_dict):
    ...
    for line in clean_data_list:
        ...
        monthly_averages[month] = (sum(months_empty_values[month]) / number_of_years)
...
def assign_daily_averages_for_specific_month(specific_months_empty_values, clean_data_list, clean_data_dict):
    ...
    for line in clean_data_list:
        month_year = line[3: 11]
        (specific_months_empty_values[month_year]).append(clean_data_dict.get(line[0:11]))
        specific_monthly_averages[month_year] = (sum(specific_months_empty_values[month_year]) / len(specific_months_empty_values[month_year]))
...
def assign_year_daily_averages(years_empty_values, clean_data_dict, clean_data_list):
    ...
    for line in clean_data_list:
        year = line[7: 11]
        (years_empty_values[year]).append(clean_data_dict.get(line[0:11]))
        year_daily_averages[year] = (sum(years_empty_values[year]) / len(years_empty_values[year]))
    return year_daily_averages

# ********************************************* PRINTING FUNCTIONS *********************************************
def print_day_with_record_rainfall(day_with_with_highest_rainfall):
    print_buffer_header_and_clear_terminal()
    print ("\nThe day with record rainfall was " + day_with_with_highest_rainfall + " with " + str(highest_rainfall * 0.01) + " inches\n")
...
def print_average_rainfall_for_day(day_averages):
    ...
    if date in day_averages:
        print ("The average rainfall for " + date + " is " + str((day_averages[date]) * 0.01) + " inches\n")
...
def print_average_rainfall_for_month(monthly_averages):
    ...
    if month in monthly_averages:
        print ("The average rainfall for " + month + " is " + str((monthly_averages[month]) * 0.01) + " inches\n")
...
def print_rainfall_for_specific_day_and_year(clean_data_dict):
    ...
    if date in clean_data_dict:
        print("The total rainfall on " + date + " was " + str((clean_data_dict[date]) * 0.01) + " inches\n")
...
def print_total_and_average_daily_rainfall_for_month_of_year(specific_monthly_averages, specific_monthly_totals):
    ...
    if month_of_year in specific_monthly_totals:
        print ("The total rainfall in " + month + " of " + year + " was " + str((specific_monthly_totals[month_of_year]) * 0.01) + " inches\n")
        print ("The average daily rainfall in " + month + " of " + year + " was " + str((specific_monthly_averages[month_of_year]) * 0.01) + " inches\n")
    print_continue_or_not()
def print_total_and_average_daily_rainfall_for_year(year_totals, year_daily_averages):
    ...
    if year in year_totals:
        print ("The total rainfall in " + year + " was " + str((year_totals[year]) * 0.01) + " inches\n")
        print ("The average daily rainfall in " + year + " was " + str((year_daily_averages[year]) * 0.01) + " inches\n")
    print_continue_or_not()
def print_total_and_average_rainfall_for_every_year(years_dict, year_totals, year_daily_averages):
    print_buffer_header_and_clear_terminal()
    for year in years_dict:
        print ("\n\n\t\t~~~ " + year + " ~~~\n")
        print ("Total rainfall:   " + str((year_totals[year]) * 0.01) + " inches")
        print ("Average daily:    " + str((year_daily_averages[year]) * 0.01) + " inches\n")
...
def call_functions_off_choice(choice):
    if choice == 'A': # Day with most rainfall
        print_day_with_record_rainfall(day_with_with_highest_rainfall)
    ...
    if choice == 'Q': # Exit program
        print_outgoing_display()
        quit()
    # if choice != 'A' or 'B' or 'C' or 'D' or 'E' or 'F' or 'G' or 'Q':
    #     print_user_options()
    #     print ("Next time, please select a valid option.\n\n")
    #     time.sleep(3)
    #     os.system('clear')
    #     cont = False
...
highest_rainfall = identify_highest_rainfall(clean_data_dict)
day_with_with_highest_rainfall = identify_day_with_highest_rainfall(highest_rainfall, clean_data_dict)

days_empty_values = assign_days_to_keys(clean_data_dict)
day_averages = assign_day_averages(days_empty_values, clean_data_dict)

months_empty_values = assign_months_to_keys(clean_data_list)
...
# specific_monthly_averages = assign_daily_averages_for_specific_month(specific_months_empty_values, clean_data_list, clean_data_dict)
#
# def assign_month_year_pair_to_keys(clean_data_list):
#     months_empty_values = {}
#     for line in clean_data_list:
#         month = line[3: 6]
#         months_empty_values[month_year] = []
#     return months_empty_values
# def assign_monthly_averages(months_empty_values, clean_data_list, clean_data_dict):
#     month_averages = {}
#     for line in clean_data_list:
#         month = line[3: 6]
#         (months_empty_values[month_year]).append(clean_data_dict.get(line[0:11]))
#         monthly_averages[month_year] = (sum(months_empty_values[month]) / len(months_empty_values[month]))
#     return monthly_averages
# months_empty_values = assign_month_year_pair_to_keys(clean_data_list)
# monthly_averages = assign_monthly_averages(months_empty_values, clean_data_list, clean_data_dict)
...
# NEXT STEPS:
# When giving specific monthly totals, afterwards display how much above or below the average it is by deducting the average from the total and make above by >= 0 or below by <= 0
# Should I put the lists/dicts into tables to practice creating that format? or move onto other problems?


#
#
# #
# def print_all_day_averages(day_averages):
