...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
card_names_to_values = {
'Two': 2, 'Three': 3, 'Four': 4, 'Five': 5, 'Six': 6, 'Seven': 7, 'Eight': 8, 'Nine': 9, 'Ten': 10,
'Jack': 10, 'Queen': 10, 'King': 10, 'Ace': 11
}
card_suits = ['Spades', 'Diamonds', 'Clubs', 'Hearts']

def initialize_all_cards_and_deck(card_suits, card_names_to_values):
    cards_in_deck = []
    for suit in card_suits:
        for name in card_names_to_values:
            cards_in_deck.append(Card(name, suit, card_names_to_values[name]))
    deck = Deck(cards_in_deck)
    return deck

def update_user_hand(current_card):
    user_hand_list.append(current_card)
    user_hand = Hand(user_hand_list)
    print('\n{:^95}'.format("Your draw:  {} of {}, worth {} points".format(current_card.name, current_card.suit, current_card.value)))
    return user_hand
def update_comp_hand(current_card):
    comp_hand_list.append(current_card)
    comp_hand = Hand(comp_hand_list)
    print('\n{:^95}'.format("Computer's draw:  {} of {}, worth {} points".format(current_card.name, current_card.suit, current_card.value)))
    return comp_hand
def display_scoreboard(user_hand, comp_hand):
    os.system('clear')
    ...
    user_card_names_printable = 'USER  >>>        ' + '   '.join(user_card_names)
    user_score_printable = 'USER  >>>          ' + user_score

    comp_card_names_printable = '   '.join(comp_card_names) + '       <<<  COMP'
    comp_score_printable = comp_score + '          <<<  COMP'
...
user_hand_list = []
comp_hand_list = []
user_hand = Hand([Card('Joker', 'Platinum', 0)])
comp_hand = Hand([Card('Joker', 'Platinum', 0)])

deck = initialize_all_cards_and_deck(card_suits, card_names_to_values)

os.system('clear')
input('{:^95}\n{:>47}'.format('Press ENTER to begin', ''))
cont = True
while cont:
    display_scoreboard(user_hand, comp_hand)
    current_card = deck.draw_from_deck()
    user_hand = update_user_hand(current_card)

    current_card = deck.draw_from_deck()
    comp_hand = update_comp_hand(current_card)
